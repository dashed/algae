...
pub fn perform(ts: TokenStream) -> TokenStream {
    let input = parse_macro_input!(ts as syn::Expr);
    quote! {{
        let __eff = algae::Effect::new((#input).into());
        let __reply_opt = yield __eff;
        __reply_opt.expect("effectful computation resumed without a Reply (the driver called resume(None) mid-computation)").take::<_>()
    }}
    .into()
}
