...
/// An operation value paired with its declared reply type.
///
/// `TypedOp` is what the constructors generated by `effect!` return: the raw
/// root operation to yield, plus a phantom `Ret` recording the reply type the
/// `effect!` declaration promised for that operation. `perform!` uses the
/// phantom to extract the reply **without a type annotation at the call
/// site** — and a request for the wrong type becomes a compile error at the
/// perform site instead of a runtime panic:
///
/// ```
/// # #![feature(coroutines, yield_expr)]
/// # use algae::prelude::*;
...
///     format!("Hello, {name}!")
/// }
/// # let result = greet().run_with(
/// #     ScriptHandler::new().reply(()).reply("Ada".to_string()));
/// # assert_eq!(result, "Hello, Ada!");
/// ```
///
/// The legacy enum form (`perform!(Console::Print(msg))` with a type
/// annotation) keeps working alongside the typed form.
pub struct TypedOp<Root, Ret> {
    op: Root,
    _ret: PhantomData<Ret>,
}

impl<Root, Ret> TypedOp<Root, Ret> {
    /// Wraps an operation with its declared reply type. Called by the
    /// constructors that `effect!` generates; rarely needed directly.
    pub fn new(op: Root) -> Self {
        Self {
            op,
            _ret: PhantomData,
        }
    ...
    /// Splits into the raw operation (to yield) and a phantom-typed
    /// [`Extractor`] (to recover the reply after resume).
    ///
    /// The split exists because extracting a field of `TypedOp` across a
    /// `yield` point would be a partial move; separating the two halves keeps
    /// the generated coroutine code borrow-checker clean.
    pub fn split(self) -> (Root, Extractor<Ret>) {
        (self.op, Extractor(PhantomData))
    }
}

/// The reply-typed half of a [`TypedOp::split`]: extracts a [`Reply`] as the
/// operation's declared reply type.
pub struct Extractor<Ret>(PhantomData<Ret>);

impl<Ret: Any + Send + 'static> Extractor<Ret> {
    /// Extracts the reply as `Ret`. Panics (like [`Reply::take`]) if the
    /// handler supplied a value of a different type.
    pub fn extract(self, reply: Reply) -> Ret {
        reply.take::<Ret>()
    }
}

/// Dispatch wrapper used by the expansion of `perform!`.
///
/// `perform!` wraps its argument in `PerformArg` and calls `.prepare()`.
/// Method resolution then picks the right path by type:
///
/// - **Typed form** (`perform!(Console::read_line())`): the argument is a
///   [`TypedOp`], the inherent `prepare` below wins, and the reply type is
///   pinned by the constructor — no annotation needed, wrong types are
///   compile errors.
/// - **Legacy form** (`perform!(Console::ReadLine)`): no inherent method
///   applies, so the [`PrepareLegacy`] trait impl is used, and the reply type
///   is inferred from context exactly as before.
///
/// Not intended for direct use.
pub struct PerformArg<T>(pub T);

impl<Root, Ret: Any + Send + 'static> PerformArg<TypedOp<Root, Ret>> {
    /// Typed path: reply type pinned by the [`TypedOp`] witness.
    pub fn prepare(self) -> (Root, Extractor<Ret>) {
        self.0.split()
    }
}

/// Fallback path for `perform!` on legacy enum operations; see [`PerformArg`].
///
/// `Root` and `R` are inferred from context (the coroutine's yield type and
/// the use of the extracted value), matching the historical semantics of
/// `perform!(Family::Variant(payload))`.
pub trait PrepareLegacy<Root, R> {
    /// Converts the operation into the root type and pairs it with an
    /// inference-driven [`Extractor`].
    fn prepare(self) -> (Root, Extractor<R>);
}

impl<Root, R: Any + Send + 'static, T: Into<Root>> PrepareLegacy<Root, R> for PerformArg<T> {
    fn prepare(self) -> (Root, Extractor<R>) {
        (self.0.into(), Extractor(PhantomData))
    }
}
...
pub mod prelude {
    pub use crate::{
        handler_fn, register_type, Effect, Effectful, Extractor, Handler, HandlerFn,
        HandlerWrapper, PartialHandler, PerformArg, PrepareLegacy, Reply, ReplyError,
        ScriptHandler, Step, TypedOp, UnhandledOp, UnhandledOpError, VecHandler,
    };
